		...
		<tr>	
			...
			<?php if( $kultalusikka_status !== false && $kultalusikka_status == 'valid' ) { ?>
				...
				<?php } else {
					...
					<input type="submit" class="button-secondary" name="kultalusikka_theme_license_activate" value="<?php _e('Activate License'); ?>" />
...
function kultalusikka_theme_activate_license() {
	...
	if( isset( $_POST['kultalusikka_theme_license_activate'] ) ) { 
		...
		$response = wp_remote_get( add_query_arg( $api_params, KULTALUSIKKA_SL_STORE_URL ) );
