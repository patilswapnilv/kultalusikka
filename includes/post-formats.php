 ...
 * @subpackage Includes
 * @note       Post format functions code are from Justin Tadlock <justin@justintadlock.com>
 * @author     Justin Tadlock <justin@justintadlock.com>
 * @link       themehybrid.com
 * @author     Sami Keijonen <sami.keijonen@foxnet.fi>
 * @since      0.1.0
 */

/* Makes URLs in link posts clickable. */
add_filter( 'the_content', 'kultalusikka_link_content' );

/* Wraps <blockquote> around quote posts. */
add_filter( 'the_content', 'kultalusikka_quote_content' );

/* Filter the content of chat posts. */
add_filter( 'the_content', 'kultalusikka_format_chat_content' );
 ...
 */
function kultalusikka_link_content( $content ) {
 ...
 * @return string $content
 */
function kultalusikka_quote_content( $content ) {
 ...
 * have 100s of speakers in your chat post, each with their own, kultalusikka classes for styling.
 *
 ...
 */
function kultalusikka_format_chat_content( $content ) {
	...
	/* Open the chat transcript div and give it a kultalusikka ID based on the post ID. */
	...
	foreach ( $chat_rows as $chat_row ) {
		...
		if ( strpos( $chat_row, $separator ) ) {
			...
			$speaker_id = kultalusikka_format_chat_row_id( $chat_author );
 ...
 * array and makes sure we have a kultalusikka set of IDs.  The purpose of this function is to provide an "ID"
 ...
 */
function kultalusikka_format_chat_row_id( $chat_author ) {
	global $_post_format_chat_ids;
 ...
 * @return string
 */
function kultalusikka_get_video_embed() {
	global $wp_embed;
	...
	add_filter( 'kultalusikka_video_shortcode_embed', array( $wp_embed, 'run_shortcode' ) );
	add_filter( 'kultalusikka_video_auto_embed', array( $wp_embed, 'autoembed' ) );
	...
	if ( is_array( $matches ) ) {
		foreach ( $matches  as $value ) {
			...
			$embed = apply_filters( 'kultalusikka_video_shortcode_embed', '[embed]' . $value[1]. '[/embed]' );
	...
	if ( empty( $embed ) && get_option( 'embed_autourls' ) ) {
		...
		if ( is_array( $matches ) ) {
			foreach ( $matches  as $value ) {
				...
				$embed = apply_filters( 'kultalusikka_video_auto_embed', $value[0] );
 ...
 */
function kultalusikka_url_grabber() {
 ...
 * @return int
 */
function kultalusikka_get_image_attachment_count() {
 ...
 */
function kultalusikka_post_has_content( $id = 0 ) {
	$post = get_post( $id );
